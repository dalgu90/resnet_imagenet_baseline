...
def train():
  ...
  with tf.Graph().as_default():
    ...
    # Input placeholder
    images = tf.placeholder(tf.float32, [FLAGS.batch_size, model.IMAGE_WIDTH, model.IMAGE_HEIGHT, 3])
    ...
    loss, acc = model.loss_acc(logits, labels)
    tf.scalar_summary('accuracy', acc)

    # Training(Regularization is performed by proximal step)
...
#         base_saver.restore(sess, ckpt.model_checkpoint_path)
    ...
    for step in xrange(init_step, FLAGS.max_steps+1):
      ...
      if step > init_step:
       ...
       try:
         train_image_val, train_label_val = sess.run([train_image, train_label])
         _, lr_value, loss_value, acc_value, train_summary_str = sess.run([train_op, lr, loss, acc, train_summary_op],
                                                                          feed_dict={images:train_image_val, labels:train_label_val},
      ...
      if step % FLAGS.test_interval == 0:
        test_image_val, test_label_val = sess.run([test_image, test_label])
        loss_value, acc_value, test_summary_str = sess.run([loss, acc, test_summary_op],
                                                           feed_dict={images:test_image_val, labels:test_label_val})
